...
class BooksRepository extends EloquentBaseRepository
{
    ...
    {
        if ($search=='') {
            $books = $this->model->with('authors', 'loans.user', 'comments')->where('owned', 'owned')->orderBy('title')->get();
        } else {
            $books = Book::with('authors', 'loans.user')->where('title', 'like', '%' . $search . '%')->where('owned', 'owned')->orderBy('title')->get();
        }
    ...
    {
        $books = $this->model->with('authors')->where('owned', 'wishlist')->orderBy('title')->get();
        return $books;
    }
    ...
    {
        $books=$this->model->where('owned', 'owned')->with('comments')->get();
        $fin=array();
        $dat=array();
        foreach ($books as $book) {
